...
import { DataProcessor } from '../../src/4-fixtures/data-processor';
import { test, testWithAutoSetup, testWithFileScope } from './test-fixtures';
...
test('should demonstrate fixture cleanup', ({ testLogger }) => {
  ...
  expect(testLogger.logs.length).toBe(5); // Включно з початковим логом
});
